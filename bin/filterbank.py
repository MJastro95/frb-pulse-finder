...
def filterbank_parse(infile, loop_index, split):
	...
	data_length= len(fileContent[:header_end_loc+10])
	data_size_per_spectra = nchans_val*nbits_val/8e6 #in megabytes

	size_per_chunk = file_size/split #in megabytes

	spectra_per_chunk = int(size_per_chunk/data_size_per_spectra) #number of spectra (frequency slices at given time) per chunk



	total_block_size = int(spectra_per_chunk*nchans_val*nbits_val/8)


	with open(infile, "rb") as file:
		file.seek(total_block_size*loop_index + data_length)
	...
	sub_int = spectra_per_chunk*tsamp_val
